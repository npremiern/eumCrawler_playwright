...
### Added
- **이미지 미리보기 기능**: 작업 목록에서 행을 선택하면 우측 미리보기 패널에서 해당 지적도 이미지를 즉시 확인 가능.
- **이미지 엑셀 최적화**: 이미지를 삽입할 때 셀 크기에 맞춰 자동으로 리사이징하고, 엑셀의 행 높이와 열 너비를 이미지에 맞춰 자동 조절하여 '셀 안에 쏙 들어가게' 개선.

### Changed
- **UI 레이아웃 확장**: 미리보기 패널 추가에 따라 기본 윈도우 너비를 800px에서 1100px로 확장.
- **이미지 앵커링**: 엑셀 내 이미지의 위치를 셀 좌측 상단에 고정하여 데이터 정렬 시에도 위치가 유지되도록 보정.

## [1.7.0] - 2026-03-17

### Added
- **UI 프레임워크 전환 (`ttkbootstrap`)**: 기존 `sv_ttk`에서 더 다채롭고 모던한 `ttkbootstrap`으로 전면 교체.
- **다이나믹 버튼 스타일**: 작업 성격(시작: Success, 중지: Danger, 저장: Info, 초기화: Warning)에 맞춘 색상 버튼 적용.
- **실시간 세부 작업 알림**: 주소별로 [이용계획 조회], [도면 저장], [PDF 저장] 등 현재 수행 중인 작업을 실시간으로 표시.
- **진행 상황 카운터 개선**: 단순 시도 횟수가 아닌, 엑셀 전체 행 대비 현재 진행률(예: 3 / 10)을 표시하도록 직관적으로 변경.
...
### Fixed
- **TclError 호환성 문제**: `ttkbootstrap`에서 발생하던 `-padding` 옵션 오류를 내부 프레임 구조로 우회하여 전면 해결.
- **레이아웃 정렬 보정**: 여백 불균형 및 수직 줄맞춤 문제를 '내부 패딩용 프레임' 도입으로 정밀하게 튜닝.
- **ID 표시 버그**: 엑셀의 NO(ID) 칸이 비어 있을 때 상단 진행 상황에 숫자가 나오지 않던 현상 수정.

### Changed
- **UI 시각 가독성**: 작업 목록(Treeview) 헤더를 굵게 변경하고, PNU/지가/상세내용 등 주요 컬럼 너비를 여유 있게 최적화.
- **컴팩트한 정렬**: 설정 영역과 버튼 영역의 수직 정렬을 맞춰 안정적인 화면 구성.
...
### Technical Details
- `crawler_gui.py`: `ttk.Window` 적용 및 `Inner Frame` 패딩 구조 도입. `total_count` 변수를 통한 진행률 계산 로직 추가.
- `crawler.py`: 세부 작업 상태(Sub-task status)를 GUI로 전달하도록 `progress_callback` 확장.
